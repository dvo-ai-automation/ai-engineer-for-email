...
try:
    ...
    sys.exit("API-call timed out")
if w.status_code in [500, 503]:
    sys.exit("Wacht even en probeer het nogmaals")
elif w.status_code in [400, 401, 404]:
    sys.exit("Ongeldige coördinaten")
elif w.status_code == 403:
    sys.exit("Je hebt geen toegang")

data = w.json()
try: 
    data["hourly"]
except KeyError:
    sys.exit("Data wel opgehaald, maar er zit niet in wat je zocht")
